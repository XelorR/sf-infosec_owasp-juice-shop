...
def flatten(object_: Object, *, path_separator: str = ".") -> Array[Object]:
    """
        ...
        for index, record in enumerate(result):
            try:
                ...
                if isinstance(value, dict):
                    ...
                    keys.update(chain.from_iterable(map(dict.keys, new_records)))
                    new_result.extend(
                        {**new_record, **record} for new_record in new_records
                    )
                else:
                    new_result.append(record)
        result = new_result
...
@singledispatch
def flatten_nested_objects(
    object_: Serializable, *, prefix: str = "", path_separator: str
) -> Object:
...
def _(object_: Array, *, prefix: str = "", path_separator: str) -> Object:
    return {
        prefix[: -len(path_separator)]: list(
            map(partial(flatten_nested_objects, path_separator=path_separator), object_)
        )
    }
